...
{
	public abstract class BaseAuthenticatedCommand<TParameter, TOutput, TUser> : BaseCommand<TParameter, TOutput> where TUser : ICommandUser
	{
		...
		protected BaseAuthenticatedCommand(IServiceProvider provider, Scopes scope) : base(provider)
		{
			_scope = scope;
